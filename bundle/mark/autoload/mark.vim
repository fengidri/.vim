...
"只是mark,如果已经mark不做任何动作  by feng
function! mark#JustMarkCurrentWord( groupNum )
	let l:regexp = (a:groupNum == 0 ? mark#CurrentMark()[0] : '')
	if empty(l:regexp)
		let l:cword = expand('<cword>')
		if ! empty(l:cword)
...
function! s:Cycle( ... )
	...
	let s:cycle = (l:newCycle < s:markNum -g:IgnoreGrougeNum ? l:newCycle : 0)
	return l:currentCycle
endfunction
function! s:FreeGroupIndex()
	let i = 0
	while i < s:markNum - g:IgnoreGrougeNum
