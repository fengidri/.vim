...
if ! exists('g:IgnoreGrougeNum')
	let g:IgnoreGrougeNum=0
endif
...
nnoremap <silent> <Plug>JustMarkSet      :<C-u>if !mark#JustMarkCurrentWord(v:count)<Bar>execute "normal! \<lt>C-\>\<lt>C-n>\<lt>Esc>"<Bar>endif<CR>
...
"by feng
command! MarkJust call mark#JustMarkCurrentWord(v:count)
command! MarkTo call mark#MarkCurrentWord(v:count)
command! MarkClearOne call mark#DoMark(v:count, (v:count ? '' : mark#CurrentMark()[0]))
